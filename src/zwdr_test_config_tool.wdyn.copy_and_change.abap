...
method ONACTIONAPPLY_CHANGES .
  data l_changes type Wdr_Context_Change_List.
  DATA: context TYPE REF TO if_wd_context.
  data l_config_modifier type ref to if_wd_config_comp_modifier.
  ...
  l_changes = context->get_context_change_log( ).

  try.
    l_config_modifier = wd_this->config_tool->get_modifier( ).
  ...
  try.
    wd_assist->apply_attribute_changes(
      attribute_changes = l_changes
      config_changer    = l_config_modifier
         ).
   catch cx_wd_config_tool into l_ex.
    lo_message_manager->report_exception(
        message_object            = l_ex
...
method ONACTIONCOPY .
  ...
  try.
    ...
    return.
  endtry.
  wd_assist->register_eventhandlers( change_tool = wd_this->config_tool message_manager = lo_message_manager ).
  ...
  l_description = wd_this->config_reader->get_description( ).
  l_node->set_attribute( name = 'DESCRIPTION' value = l_description ).
